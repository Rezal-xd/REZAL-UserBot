...
async def admin_groups(rezal):
    rezalgroups = []
    async for dialog in rezal.client.iter_dialogs():
...
async def make_gif(event, file):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            rezalresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            rezalfile = await event.client.download_media(rezalresponse, "./temp")
            return await unzip(rezalfile)
...
async def iphonex(text):
    ...
    RezalXd = r.get("message")
    rezalurl = url(RezalXd)
    if not rezalurl:
        return "check syntax once more"
    with open("temp.png", "wb") as f:
        f.write(requests.get(RezalXd).content)
...
async def waifutxt(text, chat_id, reply_to_id, bot, borg):
    ...
    rezal = await sticcers[0].click("me", hide_via=True)
    if rezal:
        await bot.send_file(int(chat_id), rezal, reply_to=reply_to_id)
        await rezal.delete()
