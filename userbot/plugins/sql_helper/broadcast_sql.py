...
class RezalBroadcast(BASE):
    __tablename__ = "rezalbroadcast"
    ...
    def __repr__(self):
        return "<Rezal Broadcast channels '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, RezalBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


RezalBroadcast.__table__.create(checkfirst=True)

RezalBROADCAST_INSERTION_LOCK = threading.RLock()
...
def add_to_broadcastlist(keywoard, group_id):
    with RezalBROADCAST_INSERTION_LOCK:
        broadcast_group = RezalBroadcast(keywoard, str(group_id))
...
def rm_from_broadcastlist(keywoard, group_id):
    with RezalBROADCAST_INSERTION_LOCK:
        broadcast_group = SESSION.query(RezalBroadcast).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_broadcastlist(keywoard):
    with RezalBROADCAST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(RezalBroadcast.keywoard)
            .filter(RezalBroadcast.keywoard == keywoard)
            .delete()
        )
...
def get_broadcastlist_chats():
    try:
        chats = SESSION.query(RezalBroadcast.keywoard).distinct().all()
...
def num_broadcastlist():
    try:
        return SESSION.query(RezalBroadcast).count()
    finally:
        SESSION.close()
...
def num_broadcastlist_chats():
    try:
        return SESSION.query(func.count(distinct(RezalBroadcast.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(RezalBroadcast).all()
        for x in all_groups:
