...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        mod.delete_rezal = delete_rezal
        mod.media_type = media_type
        # support for rezalbot originals
        sys.modules["rezalbot.utils"] = userbot.utils
        sys.modules["rezalbot"] = userbot
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.COMMAND_HAND_LER) == 2:
                rezalreg = "^" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER[1]
            elif len(Config.COMMAND_HAND_LER) == 1:
                rezalreg = "^\\" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER
            args["pattern"] = re.compile(rezalreg + pattern)
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_COMMAND_HAND_LER) == 2:
                rezalreg = "^" + Config.SUDO_COMMAND_HAND_LER
...
async def delete_rezal(event, text, time=None, parse_mode=None, link_preview=None):
    parse_mode = parse_mode or "md"
    link_preview = link_preview or False
    time = time or 5
    if event.sender_id in Config.SUDO_USERS:
        ...
        rezalevent = await event.edit(
            text, link_preview=link_preview, parse_mode=parse_mode
        )
    await asyncio.sleep(time)
    return await rezalevent.delete()
...
async def unsavegif(event, RezalXd):
    try:
        await event.client(
            functions.messages.SaveGifRequest(
                id=types.InputDocument(
                    id=RezalXd.media.document.id,
                    access_hash=RezalXd.media.document.access_hash,
                    file_reference=RezalXd.media.document.file_reference,
                ),
                unsave=True,
            )
        )
